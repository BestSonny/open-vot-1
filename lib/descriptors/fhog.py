...
def getFeatureMaps(image, k, mapp):
    ...
    func1(dx, dy, boundary_x, boundary_y, r, alfa,
          height, width, numChannels)  # with @jit
    # ~0.001s
    ...
    func2(mappmap, boundary_x, boundary_y, r, alfa, nearest,
          w, k, height, width, sizeX, sizeY, p, stringSize)
    mapp['map'] = mappmap
    # ~0.001s
...
def normalizeAndTruncate(mapp, alfa):
    ...
    func3(newData, partOfNorm, mapp['map'],
...
def PCAFeatureMaps(mapp):
    ...
    func4(newData, mapp['map'], p, sizeX, sizeY,
