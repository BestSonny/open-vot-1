                ...
                for jj in range(k):
                    if((i * k + ii > 0) and (i * k + ii < height - 1) and (j * k + jj > 0) and (j * k + jj < width - 1)):
                        ...
                        if((i + nearest[ii] >= 0) and (i + nearest[ii] <= sizeY - 1)):
                            mapp[(i+nearest[ii])*stringSize + j*p + alfa[k*i+ii,
                                                                         j*k+jj, 0]] += r[k*i+ii, j*k+jj] * w[ii, 1] * w[jj, 0]
                            mapp[(i+nearest[ii])*stringSize + j*p + alfa[k*i+ii, j*k+jj,
                                                                         1] + NUM_SECTOR] += r[k*i+ii, j*k+jj] * w[ii, 1] * w[jj, 0]
    ...
    for i in range(sizeY):
        for j in range(sizeX):
            ...
            for jj in range(2 * xp):  # 2*9
                newData[pos2 + jj] = np.sum(mappmap[pos1 +
                                                    yp*xp + jj: pos1 + 3*yp*xp + jj: 2*xp]) * ny
