...
class TrackerKCF:
    ...
    def init(self, image, roi):
        ...
        self._tmpl = self._extract_feature(image, 1)
    ...
    def update(self, image):
        ...
        loc, peak_value = self._locate_target(
            self._tmpl, self._extract_feature(image, 0, 1.0))

        if(self.scale_step != 1):
            # Test at a smaller _scale
            new_loc1, new_peak_value1 = self._locate_target(
                self._tmpl, self._extract_feature(image, 0, 1.0/self.scale_step))
            # Test at a bigger _scale
            new_loc2, new_peak_value2 = self._locate_target(
                self._tmpl, self._extract_feature(image, 0, self.scale_step))
        ...
        x = self._extract_feature(image, 0, 1.0)
    ...
    def _extract_feature(self, image, inithann, scale_adjust=1.0):
        ...
        if(inithann):
            self._create_hanning_mats()  # _create_hanning_mats need size_patch

        FeaturesMap = self.hann * FeaturesMap
        return FeaturesMap

    def _locate_target(self, z, x):
        k = self._gaussian_correlation(x, z)
        ...
        if(pi[0] > 0 and pi[0] < res.shape[1]-1):
            p[0] += self._sub_pixel_peak(res[pi[1], pi[0]-1],
                                      pv, res[pi[1], pi[0]+1])
        if(pi[1] > 0 and pi[1] < res.shape[0]-1):
            p[1] += self._sub_pixel_peak(res[pi[1]-1, pi[0]],
                                      pv, res[pi[1]+1, pi[0]])
    ...
    def _train(self, x, train_interp_factor):
        k = self._gaussian_correlation(x, x)
